...
SAFETAG 资源以知识共享署名-相同方式共享 3.0 未本地化版本 (CC BY-SA 3.0) 许可协议提供

审计框架和清单允许以教育、非商业、非营利目的使用和共享，并署名 Internews。用户可以在许可证列出的条件下自由修改和分发内容。
...
SAFETAG 本身是一个用于组织审计的框架和模板。因此，运用或改编 SAFETAG 材料进行的审计可被称作 “改编 SAFETAG 方法” 或 “基于 SAFETAG 框架”，或类似措辞，但不得称作 “SAFETAG 审计”。

这并不意味着使用了任何或所有 SAFETAG 材料的审计都需要提及 SAFETAG。

本使用政策并不影响 SAFETAG 材料的分发，这已在上述许可声明中说明。
